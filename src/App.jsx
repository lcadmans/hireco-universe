...
import { Bounds, Effects, Environment, OrbitControls, PerformanceMonitor, PerspectiveCamera, useProgress, AdaptiveDpr, AdaptiveEvents } from '@react-three/drei'
...
import { ConsoleLogger, CustomLoader } from './components'
...
function QRPanel() {
	...
	return (
		<div className={`custom-loader pointer-events-none absolute  z-[60]  flex h-full w-full items-center justify-end pr-[30vw]  transition-all delay-[200ms] duration-[800ms] ease-in-out`}>
			<div className='mt-64 flex flex-col justify-center bg-black bg-opacity-30 p-6 text-white'>
				<div
					className='back pointer-events-auto flex cursor-pointer pb-6 pt-6'
					onClick={() => {
						setQrPanel(null)
					}}
				>
				...
				<div className='bg-white p-2'>
					{/* <QRCode value='hireco.co.uk' /> */}

					<QRCodeSVG value={qrPanel} size={200} />
				</div>
				<div className=''>
					{activeRing == 'ring_2' ? (
						<>
							<p className='pt-4 tracking-wider'>Scan to connect</p>
						</>
					) : (
						<>
							<p className='pt-4 tracking-wider'>Scan to view</p>
						</>
					)}
				</div>
			</div>
...
function BackButton() {
	...
	return (
		<div
			className='back pointer-events-auto relative z-[150] flex cursor-pointer pb-6 pt-12'
			onClick={() => {
				if (activeTile) {
					setActiveTile(null)
					setFocusElementRef(null)
